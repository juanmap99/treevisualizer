...
export class MaxHeap extends StandardTree{
    
    constructor(size: number, auxService : AuxVarService){
        super(size,"MaxHeap",auxService);
    }
   ...
   */
   getParentIndex(iNode : number) : number{
    ...
    async bubbleUp(node : TreeNode){
        let biggestValue = node.valor;
        let biggestSide = "root";
        node.setColor("#48c088");
        await this.delayByRunSpeed();
        if(node.rChild && biggestValue < node.rChild?.valor){
            biggestValue = node.rChild?.valor;
            biggestSide = "right";
        }
        if(node.lChild && biggestValue < node.lChild?.valor){
            biggestValue = node.lChild?.valor;
            biggestSide = "left";
        }
        if(node.valor != biggestValue){
            if(biggestSide == "right" && node.rChild){
                this.swapValues(node.rChild,node);
            }
            else if(biggestSide == "left" && node.lChild){
    ...
    async findMin(): Promise<TreeNode> {
        ...
        if(this.root.valor == -999){
            ...
            this.auxVarServ.setAuxVariable("Current min",[this.root]);
        }
        let curMin = this.root.valor;
        let nodoMin : TreeNode = this.root;
        await this.delayByRunSpeed();
        while(queue.length != 0){
            ...
            if(curNode.valor <= curMin){
                curMin = curNode.valor;
                nodoMin = curNode;
                this.auxVarServ.setAuxVariable("Current min",[nodoMin]);
                await this.delayByRunSpeed();
            }
            if(curNode.lChild){
        ...
        this.auxVarServ.deleteAuxVar("Current min");
        nodoMin.setColor("#48c088");
        await this.delayByRunSpeedXTimes(3);
        nodoMin.setDefaultColor();
        return nodoMin;
    }
